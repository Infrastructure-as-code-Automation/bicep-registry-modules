...
@maxLength(90)
param resourceGroupName string = 'dep-${namePrefix}-azurestackhci.virtualmachineinstance-${serviceShort}-rg'
...
// Create an Arc machine within the resource group
module arcMachineDeployment 'br/public:avm/res/hybrid-compute/machine:0.4.1' = {
  name: '${uniqueString(deployment().name, enforcedLocation)}-arcMachine-${serviceShort}'
  scope: resourceGroup
  params: {
    name: '${namePrefix}${serviceShort}arcmachine'
    osType: 'Windows'
    kind: 'HCI'
    tags: {
      Environment: 'Test'
...
// Create a logical network for the network interface
module logicalNetwork 'br/public:avm/res/azure-stack-hci/logical-network:0.1.1' = {
  ...
  params: {
    ...
    tags: {
      Environment: 'Test'
      'hidden-title': 'This is visible in the resource name'
      Purpose: 'LogicalNetwork'
    }
  }
}

// Create a marketplace gallery image for the VM instance
module marketplaceGalleryImages 'br/public:avm/res/azure-stack-hci/marketplace-gallery-image:0.1.0' = {
...
// Create a network interface for the VM instance
module networkInterfaces 'br/public:avm/res/azure-stack-hci/network-interface:0.1.1' = {
  name: '${uniqueString(deployment().name, enforcedLocation)}-networkInterface-${serviceShort}'
  scope: resourceGroup
  params: {
    name: '${namePrefix}${serviceShort}nic'
    customLocationResourceId: customLocation.id
    ipConfigurations: [
      {
        properties: {
    ...
    tags: {
      Environment: 'Test'
      'hidden-title': 'This is visible in the resource name'
      Purpose: 'NetworkInterface'
    }
  }
}

module testDeployment '../../../main.bicep' = {
  name: '${uniqueString(deployment().name, enforcedLocation)}-virtualMachineInstance-${serviceShort}'
  scope: resourceGroup
  params: {
    arcMachineName: arcMachineDeployment.outputs.name
    customLocationResourceId: customLocation.id
    identity: {
      type: 'SystemAssigned'
    }
    networkProfile: {
      networkInterfaces: [
        {
          id: networkInterfaces.outputs.resourceId
        }
      ]
    }
    storageProfile: {
      imageReference: {
        id: marketplaceGalleryImages.outputs.resourceId
      }
    }
    hardwareProfile: {
      vmSize: 'Custom'
